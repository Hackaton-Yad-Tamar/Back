...
def type_count(session, start_date, end_date, city=None, type=None):
    ...
    return session.query(Request.request_type, func.count(Request.request_type)).filter(*filters).group_by(Request.request_type).all()
...
def time_taken(session, start_date, end_date, city=None, type=None):
    ...
    return session.query(Request.city, func.avg(func.date_diff(Request.created_at, Request.updated_at))).filter(*filters).group_by(Request.city).all()
...
def request_completion_time(session, start_date, end_date, city=None, request_type=None):
    ...
    return session.query(
        Request.request_id,
        (func.extract('epoch', Request_Process.completed_at) - func.extract('epoch', Request.created_at)).label("completion_time")
    ).join(Request_Process, Request.id == Request_Process.request_id).filter(*filters).all()